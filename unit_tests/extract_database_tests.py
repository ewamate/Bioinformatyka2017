...
class TestDictOps(unittest.TestCase):
    ...
    def test_extract_modomics_db(self):
        ...
        self.assertEqual(set(sum([[subkey for subkey in dict[key]] for key in dict.keys()], [])), 16)

    def test_extract_albany_db(self):
        dict = extract_albany_db()
        self.assertEqual(len(dict.keys()), 112)
        self.assertEqual(set(sum([[subkey for subkey in dict[key]] for key in dict.keys()], [])), 6)
